/**
 * SchoolSoul Payment Routing & Phone Normalization Utilities
 * Handles authoritative phone normalization, mobile network detection,
  ...
  if (country === 'UG' || cleaned.startsWith('+256') || cleaned.startsWith('256')) {
    ...
    if (!ugRegex.test(cleaned)) {
      return {
        ...
        network: 'UNKNOWN',
        countryCode: 'UG',
        error: 'Invalid Uganda mobile number. Expected format: 07XX XXX XXX or +256 7XX XXX XXX.',
      };
    }

    // Detect network
    let network: 'MTN' | 'AIRTEL' | 'UNKNOWN' = 'UNKNOWN';
    const prefix3 = cleaned.substring(4, 6); // e.g. 77, 78, 70, 75

    // MTN Uganda prefixes: 77, 78, 76, 39, 31
    if (['77', '78', '76', '39', '31'].includes(prefix3)) {
      network = 'MTN';
    }
    // Airtel Uganda prefixes: 70, 75, 74, 32
    else if (['70', '75', '74', '32'].includes(prefix3)) {
      network = 'AIRTEL';
    }
  ...
  if (country === 'KE' || cleaned.startsWith('+254') || cleaned.startsWith('254')) {
    ...
    if (!keRegex.test(cleaned)) {
      return {
        ...
        network: 'UNKNOWN',
        countryCode: 'KE',
        error: 'Invalid Kenya mobile number. Expected format: 07XX XXX XXX or +254 7XX XXX XXX.',
      };
    }

    const prefix3 = cleaned.substring(4, 6);
    let network: 'SAFARICOM' | 'AIRTEL' | 'UNKNOWN' = 'UNKNOWN';
    if (['70', '71', '72', '79', '11'].includes(prefix3)) {
      network = 'SAFARICOM';
    } else if (['73', '78'].includes(prefix3)) {
      network = 'AIRTEL';
    }
...
): { isValid: boolean; normalizedPhone: string; error?: string; warning?: string } {
  const norm = normalizePhoneNumber(rawPhone, countryCode);

  if (paymentMethod === 'MTN_MOBILE_MONEY' || paymentMethod === 'MTN') {
    if (!norm.isValid) {
      return { isValid: false, normalizedPhone: norm.normalized, error: norm.error };
    }
    if (norm.network === 'AIRTEL') {
      return {
        isValid: true,
        normalizedPhone: norm.normalized,
        warning: 'This number appears to be on Airtel Uganda. If this is an Airtel line, please select Airtel Money.',
      };
    }
    return { isValid: true, normalizedPhone: norm.normalized };
  }
  ...
  if (paymentMethod === 'AIRTEL_MONEY' || paymentMethod === 'AIRTEL') {
    ...
    if (norm.network === 'MTN') {
      return {
        isValid: true,
        normalizedPhone: norm.normalized,
        warning: 'This number appears to be on MTN Uganda. If this is an MTN line, please select MTN Mobile Money.',
      };
    }
    return { isValid: true, normalizedPhone: norm.normalized };
  }
...
export function getPaymentMethodDisplayName(method: string): string {
  switch (method?.toUpperCase()) {
    ...
    case 'PESAPAL_CARD':
    case 'CREDIT_CARD':
      return 'Credit / Debit Card (Pesapal)';
